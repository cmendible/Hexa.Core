...
namespace System
{
    ...
    public static class ObjectExtensions
    {
        ...
        public static T MakeTransient<T>(this T source)
        {
            ...
            {
                IEnumerable<PropertyInfo> referenceInfos =
                    sourceType.GetProperties(BindingFlags.Instance | BindingFlags.Public).Where(
                        p => p.PropertyType.IsSubclassOfGeneric(baseEntityType));
                foreach (PropertyInfo referenceInfo in referenceInfos)
                {
        ...
        private static void _InternalMakeTransient<T>(T source)
        {
            ...
            if (auditableInfo != null)
            {
                ...
                PropertyInfo createdAt = sourceType.GetProperty("CreatedAt", BindingFlags.Instance | BindingFlags.Public);
                createdAt.SetValue(source, default(DateTime), null);

                PropertyInfo createdBy = sourceType.GetProperty("CreatedBy", BindingFlags.Instance | BindingFlags.Public);
